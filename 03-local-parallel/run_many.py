...
if __name__ == "__main__":
    ...
    gammas = [0.2, 0.5, 1.0, 2., 5.]  # Where do we want to iterate over?
    client = dask.distributed.Client(
        threads_per_worker=1, n_workers=len(gammas)
    )  # Define a "cluster" locally, where each simulation gets 1 (logical) core, but we spawn len(gammas) of them
    for gamma in gammas:
        config_updates = {"gamma": gamma}
        lazy_results.append( # delayed only tells dask that this is a task it should compute eventually, but for now, let's just say sometime
            dask.delayed(run_one)(config_updates)
        )
